...
import jamorham.keks.Digest;
import jamorham.keks.SHA256;
...
public class Calc {
    ...
    public static class ZKP {
        private BigInteger exponent = Curve.getExponent();
    ...
    public static boolean validateZeroKnowledgeProof(final ECPoint g, final ECPoint publicKey, final ECPoint gv, BigInteger b, byte[] party) {
        if (b == null) {
            System.out.println("Validating ZKP: b is null → false");
            return false;
        }

        BigInteger hash = getZeroKnowledgeHash(g, gv, publicKey, party);

        System.out.println("Validating ZKP:");
        System.out.println(" g.X:" + g.getXCoord().toBigInteger().toString(16));
        System.out.println(" g.Y:" + g.getYCoord().toBigInteger().toString(16));
        System.out.println(" gv.X:" + gv.getXCoord().toBigInteger().toString(16));
        System.out.println(" gv.Y:" + gv.getYCoord().toBigInteger().toString(16));
        System.out.println(" publicKey.X:" + publicKey.getXCoord().toBigInteger().toString(16));
        System.out.println(" publicKey.Y:" + publicKey.getYCoord().toBigInteger().toString(16));
        System.out.println(" b (proof):" + b.toString(16));
        System.out.println(" party:" + bytesToHex(party));
        System.out.println(" h:" + hash.toString(16));

        ECPoint term1 = g.multiply(b);
        ECPoint term2 = publicKey.multiply(hash);
        ECPoint left = term1.add(term2).normalize();

        System.out.println(" left.X:" + left.getXCoord().toBigInteger().toString(16).substring(0, 20) + "...");
        System.out.println(" gv.X == left.X ?" + left.getXCoord().toBigInteger().equals(gv.getXCoord().toBigInteger()));
        System.out.println(" left.Y == gv.Y ?" + left.getYCoord().toBigInteger().equals(gv.getYCoord().toBigInteger()));
    
        boolean valid = left.equals(gv);
        System.out.println(" left == gv ? " + valid);

        boolean sameValid = g.multiply(b)
                .add(publicKey.multiply(hash))
                .normalize()
                .equals(gv);

        System.out.println(" valid == origValid ? " + (sameValid = valid));
    ...
    public static BigInteger getZeroKnowledgeHash(final ECPoint g, final ECPoint gv, final ECPoint gx, byte[] party) {
        ...
        System.out.println(" digestBytes:" + bytesToHex(digestBytes));
