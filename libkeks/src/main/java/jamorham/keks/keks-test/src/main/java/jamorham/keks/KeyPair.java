...
public class KeyPair {
    ...
    /**
     * Create KeyPair from a private key (BigInteger or hex string)
     */
    public static KeyPair fromPrivateKey(BigInteger privateKey) {
        if (privateKey == null) return null;
        try {
            ECPoint publicKey = Curve.G.multiply(privateKey).normalize();
            return new KeyPair(privateKey, publicKey);
        } catch (Exception e) {
            Log.l("Failure to generate KeyPair from private key");
            return null;
        }
    }

    /**
     * Create KeyPair from a private key hex string
     */
    public static KeyPair fromPrivateKey(String privateKeyHex) {
        ...
        try {
            ...
            Log.l("Failure to generate KeyPair from private key hex");
            return null;
        }
    }
